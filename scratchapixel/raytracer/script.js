...
function trace(rayorig, raydir, spheres, depth) {
  ...
  if ((sphere.transparency > 0 || sphere.reflection > 0) && depth < MAX_RAY_DEPTH) {
    ...
    var refldir = raydir.sub(nhit).mult(2).mult(raydir.dot(nhit));
    refldir.normalize();
    ...
    surfaceColor = sphere.surfaceColor.mult(
      reflection * fresneleffect + refraction * (1- fresneleffect) * sphere.transparency);
  }
  else {
...
function render(spheres) {
    ...
    var fov = 30, aspectratio = width / height;
